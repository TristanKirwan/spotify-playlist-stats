...
export default function PlaylistIntro({
  ...
  if (!Array.isArray(initialStats)) finishCallback();
  ...
  return (
    <div className="fixed top-0 left-0 flex items-center justify-center w-full h-screen">
      ...
      <div
        className="z-10 grid overflow-hidden opacity-0"
        ref={statsContainerRef}
      >
        {initialStats.map((stat) => (
          <div
            className="flex flex-col col-start-1 row-start-1 gap-2"
            key={stat.label}
          >
            <span className="gap-2 text-center opacity-0 font-barlow-bold text-heading-1 val">
              {stat.value}
            </span>
            <span className="gap-2 text-center opacity-0 text-heading-3 label">
              {stat.label}
            </span>
          </div>
        ))}
      ...
      <h1
        className="absolute w-full px-4 text-center text-black -translate-x-1/2 -translate-y-1/2 opacity-0 top-1/2 left-1/2 font-barlow-bold text-heading-1"
