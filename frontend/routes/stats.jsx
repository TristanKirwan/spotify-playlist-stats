      ...
      <div className="h-full min-h-screen flex items-center">
        {data?.tracks ? (
          <>
            {showIntro ? (
              <PlaylistIntro
                initialStats={initialPlaylistStats}
                playlistName={data.name}
                finishCallback={() => setShowIntro(false)}
              />
            ) : (
              <ChartView playlistName={data?.name} data={data?.tracks} />
            )}
          </>
        ) : (
          <Container containerClass="py-12 w-full">
            <div className="flex flex-col items-center gap-12 md:flex-row md:items-start md:justify-center">
              ...
              <div
                className="w-4/12 max-w-xs scale-x-100 opacity-0 md:max-w-tiny xl:max-w-xxs"
